...
async function install(ctx, logger, config, disposers) {
  ...
  async function loadPlan(sessionId, hintSession) {
    let plan = planCache.get(sessionId)
    if (plan === undefined) {
      const p = planPath(sessionId)
      const saved = p ? await readJson(p) : null
      logLine('loadPlan: session=' + sessionId + ' saved=' + (saved ? 'yes' : 'no') + ' hint=' + (hintSession ? typeof hintSession : 'none'))
      ...
      // 新会话（尚无任何历史）：自动注入启用中的规约记忆；旧会话非常驻，可手动加入
      if (!saved) {
        try { await autoInjectConventions(sessionId, plan, hintSession) } catch (e) { logLine('loadPlan: autoInject threw: ' + String((e && e.message) || e)) }
      }
    }
    return plan
  }
  ...
  // 从所有会话的注入计划（planCache + pinned/*.json）中移除指定记忆（禁用记忆时全局生效）
  async function purgeMemoryFromPlans(id) {
    for (const [sid, plan] of planCache) {
      if (!Array.isArray(plan.memories) || !plan.memories.some((m) => String(m.id) === id)) continue
      plan.memories = plan.memories.filter((m) => String(m.id) !== id)
      try { await savePlan(sid, plan) } catch { /* ignore */ }
    }
    const dir = libPath()
    if (!dir) return
    const pinDir = norm(dir) + '/pinned'
    const entries = await listDir(pinDir)
    for (const e of entries) {
      if (!e || e.type !== 'file' || !/\.json$/.test(String(e.name))) continue
      const sid = String(e.name).replace(/\.json$/, '')
      if (planCache.has(sid)) continue
      const plan = await readJson(pinDir + '/' + e.name)
      if (!plan || !Array.isArray(plan.memories) || !plan.memories.some((m) => String(m.id) === id)) continue
      plan.memories = plan.memories.filter((m) => String(m.id) !== id)
      try { await writeJson(pinDir + '/' + e.name, { pinned: plan.pinned || [], memories: plan.memories, excluded: plan.excluded || [], injectOnce: plan.injectOnce === true }) } catch { /* ignore */ }
    }
  }
  ...
  async function conventionMemories() {
    ...
    for (const mem of memoryIndex.values()) {
      ...
      if (mem.meta.autoInjectRejected === true) continue
      out.push({
        id: String(mem.meta.id),
  ...
  // 把规约记忆 + 最近 N 条会话总结记忆加入新会话计划（仅首次创建计划时调用；返回是否发生了注入）
  async function autoInjectConventions(sessionId, plan, hintSession) {
    logLine('autoInject: ' + sessionId + ' enabled=' + cfg.enabled + ' autoInjectConvention=' + cfg.autoInjectConvention)
    if (!cfg.enabled || cfg.autoInjectConvention === false) return false
    const fresh = await isNewSession(sessionId, hintSession)
    logLine('autoInject: isNewSession=' + fresh)
    ...
    push(summs)
    if (added) {
      // 并发防护：autoInject 的异步链（isNewSession/scanLibrary）期间用户可能已操作计划
      ...
      if (current && typeof current === 'object' && Array.isArray(current.memories)) {
        planCache.set(sessionId, current)
  ...
  async function excludeTurn(session, sessionId, turnId, exclude) {
    ...
    if (exclude) {
      ...
      // 并顺带从 excluded 中移除；一个标记都没有才视为无需恢复。这样即使 plan.excluded 与
    ...
    try {
      ...
      switch (op) {
        ...
        case 'session.summarize': {
          ...
          if (!dir) return { error: '记忆库未配置' }
          const plan = await loadPlan(sessionId)
          const allTurns = buildTurns(view.session, plan)
        ...
        case 'plan.addMemory': {
          ...
          // 手动加入 = 用户重新接受自动注入，清除「已移除」标记
          if (mem.meta.autoInjectRejected === true) {
            const meta = { ...mem.meta, autoInjectRejected: false }
            meta.revision = Number(meta.revision || 1) + 1
            meta.updatedAt = now()
            try { await writeMemory(meta, mem.snapshot, mem.notes) } catch (e) { logLine('plan.addMemory: un-reject failed: ' + String((e && e.message) || e)) }
          }
        ...
        case 'plan.removeMemory': {
          ...
          plan.memories = (plan.memories || []).filter((m) => String(m.id) !== id)
          await savePlan(sessionId, plan)
          // 自动注入类记忆（规约/会话总结）被移除 → 打标记，之后不再自动注入任何新会话；
          // 想恢复自动注入可在记忆库手动「加入计划」（addMemory 会清除标记）。
          const mem = memoryIndex.get(id)
          if (mem && Array.isArray(mem.meta.tags) && (mem.meta.tags.map(String).includes(CONVENTION_TAG) || mem.meta.tags.map(String).includes(SUMMARY_TAG))) {
            const meta = { ...mem.meta, autoInjectRejected: true }
            meta.revision = Number(meta.revision || 1) + 1
            meta.updatedAt = now()
            try { await writeMemory(meta, mem.snapshot, mem.notes) } catch (e) { logLine('plan.removeMemory: reject mark failed: ' + String((e && e.message) || e)) }
          }
          return { ok: true }
        }
  ...
  if (agentsSvc !== undefined) {
    ...
    disposers.push(ctx.on('agent/created', (payload) => {
      ...
      if (payload && payload.agent && payload.agent.id) {
        // 传入 live session 供"新会话"判定（新会话无历史事件 → 自动注入规约记忆）
        loadPlan(String(payload.agent.id), payload.agent.session).then((plan) => {
          logLine('loadPlan done for ' + payload.agent.id + ' memories=' + (plan && plan.memories ? plan.memories.length : '?'))
