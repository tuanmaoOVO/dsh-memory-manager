...
async function install(ctx, logger, config, disposers) {
  ...
  // ================= 会话级一次性注入记忆存储（pinned/once.json） =================
  // 「选中记忆 → 直接发起一次上下文注入」：不入全局计划，仅当前会话的下一次请求注入，注入后自动清除。
  const onceCache = new Map() // sessionId -> [{ id, title, impressions, tags }]
  const ONCE_FILE = () => {
    const dir = libPath()
    return dir ? norm(dir) + '/pinned/once.json' : null
  }
  async function loadOnce(sessionId) {
    if (!sessionId) return []
    if (onceCache.has(sessionId)) return onceCache.get(sessionId)
    const p = ONCE_FILE()
    let list = []
    if (p) {
      const data = await readJson(p)
      if (data && Array.isArray(data[sessionId])) list = data[sessionId]
    }
    onceCache.set(sessionId, list)
    return list
  }
  function onceOf(sessionId) { // 同步读取（内存缓存；未加载时为空，点击注入/预载会填充）
    return sessionId && onceCache.has(sessionId) ? onceCache.get(sessionId) : []
  }
  async function saveOnce(sessionId, list) {
    if (!sessionId) return
    onceCache.set(sessionId, list)
    const p = ONCE_FILE()
    if (!p) return
  ...
  function renderContextFor(sessionId) {
    ...
    const onceList = sessionId ? onceOf(sessionId) : []
    ...
    if (onceList.length) {
      onceCache.set(sessionId, [])
      saveOnce(sessionId, []).catch(() => {})
    }
    ...
    try {
      ...
      switch (op) {
        case 'state.get': {
          ...
          return {
            ...
            plan: plan ? {
              ...
              once: sessionId ? await loadOnce(sessionId) : [],
        ...
        case 'session.injectNow': {
          // 直接注入：立即把记忆内容插入当前会话对话流（user 消息，带【记忆注入】标记），
          // 模型的下一次响应必然基于它；与"随新消息注入"（system prompt 一次性）不同，
          // 本条消息直接成为会话上下文的一部分。
          ...
          const view = await sessionView(sessionId)
          if (!view) return { error: '会话不存在或不在线' }
          if (!view.writable) return { error: '直接注入需要可写的活动会话（live session）' }
          ...
          try {
            view.session.append('user/message',
              { id: 'mem-inject-' + rand(10), role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text }] },
              { surfaceOp: 'append' })
          } catch (e) {
            return { error: '注入失败: ' + String((e && e.message) || e) }
          }
          logLine('session.injectNow: ' + sessionId + ' <- ' + id)
        ...
        case 'plan.injectOnceMemory': {
          // 选中记忆 → 当前会话一次性注入（不入全局计划；下一次请求渲染后自动清除）
          if (!sessionId) return { error: '缺少会话 id' }
          const id = String(a.id || '')
          const mem = memoryIndex.get(id)
          if (!mem) return { error: '记忆不存在: ' + id }
          if (mem.meta.enabled === false) return { error: '该记忆已禁用' }
          const once = await loadOnce(sessionId)
  ...
  if (agentsSvc !== undefined) {
    ...
    disposers.push(ctx.on('agent/created', (payload) => {
      ...
      if (payload && payload.agent && payload.agent.id) {
        ...
        // 预载该会话的排除记录与一次性注入记录（供消息页/注入渲染同步读取）
        loadExcluded(String(payload.agent.id)).catch(() => {})
        loadOnce(String(payload.agent.id)).catch(() => {})
      }
    }, { global: true }))
    try {
