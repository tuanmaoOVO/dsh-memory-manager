...
// 调试日志落在 DSH 主目录（跨平台：Windows / Linux / WSL 均可用），可用 DSH_HOME 环境变量覆盖
const DSH_HOME = process.env.DSH_HOME || join(homedir(), '.dsh')
const LOG_FILE = join(DSH_HOME, 'memory-manager-boot.log')
const CLIENT_LOG_FILE = join(DSH_HOME, 'memory-manager-client.log')
...
async function install(ctx, logger, config, disposers) {
  ...
  // - 注入与「用户发送」绑定：仅当本次请求由用户消息（source.kind==='user'）触发时
  //   注入一次——一次发送 = 一次可消费的注入；工具循环后续 step / 恢复重放 step 的
  //   claimed 不含用户消息，不再重复注入（实测恢复触发 7 次重复快照）
  disposers.push(ctx.on('agent/pre-step', async (payload, next) => {
    ...
    try {
      ...
      const hasUserMsg = decision.messages.some((m) => m && m.source && m.source.kind === 'user')
