...
async function install(ctx, logger, config, disposers) {
  ...
  // ================= 注入渲染 =================
  // 纯渲染：不消费 once 队列 / 不复位 injectOnce（消费由 pre-step 在确定注入后调用）
  ...
  // 注入成功后消费一次性状态（once 队列 / 仅本次发送注入）；返回消费后再渲染的文本，
  // 供 pre-step 预置去重键——避免 once 消费导致的内容变化在同一批 step 里触发第二次注入
  function consumeInjectState(sessionId) {
    const plan = planCache.get('global')
  ...
  //   导致注入内容从未进入模型请求（2026-08-15 实测：injectOnce 有 consumed 日志但 system 无内容）
  // - 不依赖 runtime context：不受 includeRuntimeContext:false 门控
  // - form='snapshot' 让 UI 折叠显示（同 time-context），模型可见、不污染对话流
  // - 每个请求 step 触发一次；内容去重：与上次注入文本相同则跳过（快照已在会话历史，
  //   模型持续可见），避免恢复/多 step 请求时重复注入同一条上下文（实测恢复触发 7 次）
  const lastInjected = new Map() // sessionId -> 上次注入文本
  disposers.push(ctx.on('agent/pre-step', async (payload, next) => {
    ...
    try {
      const agent = payload && (payload.agent || payload.scope)
      const sessionId = agent && agent.id ? String(agent.id) : null
      if (!sessionId) return decision
      const text = renderContextFor(sessionId)
      if (!text) return decision
      if (lastInjected.get(sessionId) === text) return decision
      // 注入成功后才消费一次性状态（once 队列 / 仅本次发送注入）；
      // 预置去重键为「消费后再渲染的文本」，避免 once 消费导致的内容变化触发第二次注入
      const after = consumeInjectState(sessionId)
      lastInjected.set(sessionId, after || text)
      logLine('injectOnce consumed: session=' + sessionId + ' chars=' + text.length)
      return {
        ...decision,
