...
function PlanTab(props) {
  ...
  return h("div", null, [
    h("div", { key: "intro", className: "mcard" },
      ...
      h("div", { className: "msetting-desc" }, "本会话的注入计划 = 下方「固定消息」+「勾选记忆」。开启「临时记忆模式」后，每次发送都会把它们注入模型上下文（" + (cfg.mode === "on" ? "当前已开启" : "当前未开启") + "）；本页可随时查看、添加与移除。")),
  ...
  return h(Boundary, { tag: "library-tab" },
    h("div", null, [
      ...
      h("div", { key: "compose", style: { display: "flex", gap: 6, alignItems: "center", marginTop: 2 } },
        h("span", { className: "mstatus" }, selected.length ? "已选 " + selected.length + " 条" : "「加入计划」= 加入本会话注入计划（开启临时记忆模式后每轮注入，可在「计划」页管理）；勾选多条可组合为新记忆（源记忆保留）"),
          ...
          h("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
            ...
            (() => {
              ...
              const title = m.enabled === false ? "该记忆已禁用，先启用再加入计划" :
                (inPlan ? "已加入本会话注入计划：开启「临时记忆模式」后每轮注入模型上下文；可在「计划」页移除" :
                  "加入本会话注入计划：开启「临时记忆模式」后每轮自动注入模型上下文，可在「计划」页管理");
      ...
      h("div", { className: "mdlg", onClick: (e) => e.stopPropagation() },
        ...
        h("div", { style: { display: "flex", gap: 6, margin: "4px 0" } },
          ...
          planMemIds.has(String(detail.meta.id))
            ? h(Btn, { sm: true, kind: "ghost", disabled: true, title: "已加入本会话注入计划：开启「临时记忆模式」后每轮注入模型上下文；可在「计划」页移除" }, "✓ 已在计划中")
            : h(Btn, { sm: true, kind: "primary", onClick: async () => { const r = await call("plan.addMemory", { id: detail.meta.id }); if (!r.error) onRefresh(); }, title: "加入本会话注入计划：开启「临时记忆模式」后每轮自动注入模型上下文" }, "加入计划")),
