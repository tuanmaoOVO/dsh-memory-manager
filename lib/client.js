...
const CSS = [
  ...
  ".mtag.auto{opacity:1;background:var(--dsw-alias-state-success-primary,#22c55e);color:var(--dsw-alias-label-primary-foreground,#fff)}",
...
const Empty = (text, key) => h("div", { key, className: "mstatus", style: { padding: "18px 0", textAlign: "center" } }, text);
...
function PlanTab(props) {
  ...
  const cancelOnce = async (id) => { const r = await call("plan.injectOnceMemory", { id, cancel: true }, currentSessionId || panel.sessionId); if (!r.error) props.refresh(); };
  ...
  return h("div", null, [
    ...
    (plan.pinned || []).length === 0 ? Empty("暂无固定消息：在对话中点击消息下方的 📌 图标即可固定", "pins-empty") :
    ...
    (plan.memories || []).length === 0 ? Empty("暂无勾选记忆：到「记忆库」点「加入计划」，或由新会话自动注入规约记忆", "mems-empty") :
    ...
    h("div", { key: "onceq", className: "msep" }, "待一次性注入（" + (plan.once || []).length + " · 下一次发送时注入，注入后自动清除）"),
    (plan.once || []).length === 0 ? Empty("在「记忆库」页点「注入一次」可排队一条记忆：下一次发送时注入（任意会话），不入计划", "once-empty") :
      (plan.once || []).map((m) => m ? h("div", { key: "once-" + m.id, className: "mrow" },
        h("span", { className: "mtag auto", title: "一次性注入：下一次发送时注入后自动清除（任意会话）" }, "一次性"),
        h("span", { className: "grow mtext", title: m.title }, m.title),
        h("Btn", { sm: true, kind: "ghost", onClick: () => cancelOnce(m.id), title: "移除：该记忆不再随下一次发送注入" }, "移除")) : null),
    h("div", { key: "excl", className: "msep" }, "已排除的轮次（" + (plan.excluded || []).length + " · 不注入模型，点击恢复）"),
    (plan.excluded || []).length === 0 ? Empty("无排除：到「消息」页点轮次头的「排除」按钮（不删除消息，随时可恢复）", "excl-empty") :
...
function LibraryTab(props) {
  ...
  // 一次性注入：下一次发送时注入该记忆（全局队列，不入计划），注入后自动清除
  const injectOnce = async (id, title) => {
    ...
    setOnceTip(r.error ? ("注入失败: " + r.error) : ("已加入一次性注入：下一次发送时注入「" + (title || id) + "」（不入计划）"));
    if (!r.error) props.refresh();
    setTimeout(() => setOnceTip(""), 4000);
  };
  // 移除一次性注入（全局队列；取消后不再随下一次发送注入）
  const cancelOnce = async (id) => {
    const sid = currentSessionId || panel.sessionId;
    const r = await call("plan.injectOnceMemory", { id, cancel: true }, sid);
    setOnceTip(r.error ? ("移除失败: " + r.error) : "已移除一次性注入：该记忆不再随下一次发送注入");
    if (!r.error) props.refresh();
    setTimeout(() => setOnceTip(""), 3000);
  };
  // 直接注入：把记忆注入当前所在会话的模型上下文（不显示为对话消息），并立即唤醒模型基于它响应
  const injectNow = async (id, title) => {
    ...
    setOnceTip(r.error ? ("直接注入失败: " + r.error) : ("已注入当前会话上下文：记忆「" + (title || id) + "」已生效，模型将立即基于它继续对话"));
    setTimeout(() => setOnceTip(""), 6000);
  };
        ...
        shown.map((m) => m ? h("div", { key: m.id, className: "mcard" },
          h("div", { style: { display: "flex", alignItems: "center", gap: 6 } },
            ...
            (() => {
              ...
              const inOnce = (props.onceIds || []).some((o) => o && String(o.id) === String(m.id));
            ...
            (() => {
              const inOnce = (props.onceIds || []).some((o) => o && String(o.id) === String(m.id));
              return inOnce
                ? h(Btn, { sm: true, kind: "outline", onClick: () => cancelOnce(m.id), title: "移除一次性注入：该记忆不再随下一次发送注入" }, "✕ 移除注入")
                : h(Btn, { sm: true, kind: "ghost", onClick: () => injectOnce(m.id, m.title), title: "注入一次：下一次发送时把该记忆注入模型上下文（不入计划）" }, "注入一次");
            })(),
...
function MessagesTab(props) {
  ...
  return h(Boundary, { tag: "messages-tab" },
    h("div", null, [
      ...
      !sessId ? Empty("选择上方会话以查看其消息（未选择时不会加载数据）", "no-sess") :
        (turns.length === 0 && !loading ? Empty("暂无消息", "no-turns") : null),
...
function Panel(props) {
  ...
  const switchTab = (t) => {
    ...
    if (t === "plan" || t === "messages" || t === "library") refresh();
  };
  ...
  else if (tab === "library") content = h(Boundary, { tag: "library-tab", onCrash, onReset: resetToPlan }, h(LibraryTab, { memories: mems, planMemories: plan ? plan.memories : [], onceIds: plan ? plan.once : [], refresh, onTraceToMsg }));
