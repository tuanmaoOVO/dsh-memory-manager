  ...
  return h("div", null, [
    ...
    (plan.once || []).length === 0 ? Empty("在「记忆库」页点「注入一次」可排队一条记忆：下一次发送时注入（任意会话），不入计划", "once-empty") :
      (plan.once || []).map((m) => m ? h("div", { key: "once-" + m.id, className: "mrow" },
        ...
        h("span", { className: "grow mtext", title: m.title }, m.title),
        h(Btn, { sm: true, kind: "outline", onClick: () => cancelOnce(m.id), title: "移除：该记忆不再随下一次发送注入" }, "移除")) : null),
...
function LibraryTab(props) {
  ...
  const injectOnce = async (id, title) => {
    ...
    setOnceTip(r.error ? ("注入失败: " + r.error) : ("已加入一次性注入：下一次发送时注入「" + (title || id) + "」（不入计划；可在「计划」页待一次性注入中移除）"));
    ...
    h("div", null, [
      ...
      shown.length === 0 ? Empty("记忆库为空：在对话中点消息下方的 💾 图标保存为记忆") :
        shown.map((m) => m ? h("div", { key: m.id, className: "mcard" },
          ...
          // 第二行：印象 + 标签
          h("div", { className: "mimpr", style: { marginTop: 3, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" } },
            h("span", null, "印象: " + ((m.impressions || []).join("、") || "—") + (m.enabled === false ? " · 已禁用" : "")),
            tagList(m.tags).length ? h("span", { className: "mtagrow", style: { marginTop: 0 } }, TagBadges(tagList(m.tags))) : null),
          // 第三行：操作按钮（居右）
          h("div", { style: { display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 6, marginTop: 6, flexWrap: "wrap" } },
            (() => {
            ...
            h(Btn, { sm: true, kind: "ghost", onClick: () => injectOnce(m.id, m.title), title: "注入一次：下一次发送时把该记忆注入模型上下文（不入计划；可在「计划」页待一次性注入中移除）" }, "注入一次"),
