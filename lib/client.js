...
function PlanTab(props) {
  ...
  return h("div", null, [
    ...
    h("div", { key: "onceq", className: "msep" }, "待一次性注入（" + (plan.once || []).length + " · 当前会话下次发送注入后自动清除）"),
    (plan.once || []).length === 0 ? Empty("在「记忆库」页点「注入一次」可排队一条记忆：下次发送时注入当前会话，不入计划", "once-empty") :
      (plan.once || []).map((m) => m ? h("div", { key: "once-" + m.id, className: "mrow" },
        h("span", { className: "mtag auto", title: "一次性注入：下次发送注入后自动清除" }, "一次性"),
        h("span", { className: "grow mtext", title: m.title }, m.title),
